...
!!! Warning "Network Problems"
    ...
    But mirror repo has a certain **latency**.
...
=== "Mirror"
    ...
    ```bash
    helm repo add prometheus-community https://prometheus-community.github.io/helm-charts
    sed -i 's/\(https:\/\/.*tgz$\)/https:\/\/git.zsnmwy.workers.dev\/\1/g' ~/.cache/helm/repository/prometheus-community-index.yaml
    helm install prometheus -n crane-system \
                            --set pushgateway.enabled=false \
                            --set alertmanager.enabled=false \
                            --set server.persistentVolume.enabled=false \
                            -f https://zsnmwy.coding.net/p/gocrane/d/helm-charts/git/raw/main/integration/prometheus/override_values.yaml?download=false \
                            --create-namespace  prometheus-community/prometheus
    ```
...
=== "Mirror"
    ...
    ```bash
    helm repo add grafana https://grafana.github.io/helm-charts
    sed -i 's/\(https:\/\/.*tgz$\)/https:\/\/git.zsnmwy.workers.dev\/\1/g' ~/.cache/helm/repository/grafana-index.yaml
    helm install grafana \
                 -f https://zsnmwy.coding.net/p/gocrane/d/helm-charts/git/raw/main/integration/grafana/override_values.yaml?download=false \
                 -n crane-system \
                 --create-namespace grafana/grafana
    ```
...
=== "Mirror"
    ...
    ```bash
    helm repo add crane https://helm-charts.gocrane.zsnmwy.net
    helm install crane -n crane-system --create-namespace crane/crane
    helm install fadvisor -n crane-system --create-namespace crane/fadvisor
    ```
...
=== "Mirror"
    ...
    ```bash
    git clone https://e.coding.net/zsnmwy/gocrane/crane.git
    CRANE_LATEST_VERSION=$(curl -s https://api.github.com/repos/gocrane/crane/releases/latest | grep -oP '"tag_name": "\K(.*)(?=")')
    git checkout $CRANE_LATEST_VERSION
    kubectl apply -f deploy/manifests
    kubectl apply -f deploy/craned
